...
class ProjectResourceIT {
    ...
    @Test
    @Transactional
    void getAllProjectsByCreatorIsEqualToSomething() throws Exception {
        User creator;
        ...
        // Get all the projectList where creator equals to "invalid-id"
        defaultProjectShouldNotBeFound("creatorId.equals=" + "invalid-id");
    }
    ...
    @Test
    @Transactional
    void getAllProjectsByMembersIsEqualToSomething() throws Exception {
        User members;
        if (TestUtil.findAll(em, User.class).isEmpty()) {
            projectRepository.saveAndFlush(project);
            members = UserResourceIT.createEntity();
        } else {
            members = TestUtil.findAll(em, User.class).get(0);
        }
        em.persist(members);
        em.flush();
        project.addMembers(members);
        projectRepository.saveAndFlush(project);
        String membersId = members.getId();
        // Get all the projectList where members equals to membersId
        defaultProjectShouldBeFound("membersId.equals=" + membersId);

        // Get all the projectList where members equals to "invalid-id"
        defaultProjectShouldNotBeFound("membersId.equals=" + "invalid-id");
    }
